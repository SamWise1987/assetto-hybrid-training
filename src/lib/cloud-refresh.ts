...
import { pullAthleteProfileFromCloud, pullExternalWorkoutsFromCloud, pullHealthMetricsFromCloud } from "./remote-sync";
...
export async function refreshAthleteCloudState(): Promise<AthleteCloudRefreshResult> {
  ...
  return {
    ...
    healthMetricCount: results[1].status === "fulfilled" ? results[1].value.length : 0,
    normalizedItemCount: results[2].status === "fulfilled" ? results[2].value : 0,
    assignedPlan: results[3].status === "fulfilled" ? results[3].value : null,
