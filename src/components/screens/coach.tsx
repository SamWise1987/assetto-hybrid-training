...
import type { TrainingPlan, TrainingPlanSession } from "@/lib/types";
...
export function CoachScreen() {
    ...
    return (
      <div className="screen-stack">
        <header className="section-heading">
          ...
          <h1>Studio coach</h1>
          <p>Accedi con un account admin o coach per rinominare gli allenamenti e assegnare piani.</p>
        </header>
      </div>
    );
  }
  ...
  const savePlan = async () => {
    ...
    if (token) {
      ...
      if (!response.ok) {
        const error = (await response.json().catch(() => ({ error: "Salvataggio cloud fallito" }))) as { error?: string };
        setStatus(error.error ?? "Salvataggio cloud fallito.");
        return;
      }
      const body = (await response.json()) as { plan: TrainingPlan };
      await saveTrainingPlan(body.plan);
      setSelectedPlanId(body.plan.id);
      setStatus("Piano salvato su cloud e applicato in locale.");
      return;
    }

    await saveTrainingPlan(plan);
    setSelectedPlanId(plan.id);
    setStatus("Piano salvato in locale.");
  };
  ...
  const assignPlan = async () => {
    ...
    const token = await getRemoteAccessToken();
    if (!token) {
      setStatus("Per assegnare serve login cloud.");
      return;
    }
    ...
    const body = (await response.json()) as { assignment: { id: string; planId: string; athleteEmail: string; assignedBy: string; assignedAt: string; active: boolean }; plan: TrainingPlan | null };
  ...
  return (
    <div className="screen-stack">
      <header className="section-heading">
        <p className="date-label">{account?.role === "admin" ? "Admin" : "Coach"}{account?.email ? ` · ${account.email}` : ""}</p>
        <h1>Studio piani</h1>
        <p>Rinomina gli allenamenti e assegna schede personalizzate agli atleti.</p>
      </header>
      ...
      <Surface>
        <div className="surface-heading">
          <div><p className="date-label">Assegnazione</p><h2>Invia piano all'atleta</h2></div>
          <UserRoundCog />
        </div>
        <Field label="Email atleta" type="email" value={assignEmail} onChange={(e) => setAssignEmail(e.target.value)} />
